...
def add_task(omnifocus_document, new_task_properties)
  ...
  if $DEBUG
    puts "JOFSYNC.add_task: going to check for existing tasks with the same name prefix: " + name
  end
  ...
  if $opts[:inbox]
    ...
    exists = proj.tasks.get.find { |t| t.name.get.force_encoding("UTF-8").start_with?(name) }
    if $DEBUG
      puts "JOFSYNC.add_task: task exists = " + exists.to_s()
    end
  end
